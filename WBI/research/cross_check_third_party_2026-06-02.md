# WBI — Cross-Check: Third-Party Sources

**Date:** 2026-06-02
**Agent:** Marvin (automated fill)
**Marvin dive:** `WBI/research/deep_dive_2026-06-02.md`
**Source inventory:** `WBI/third-party-analyses/source_inventory_2026-06-02.md`
**Framework:** `_system/frameworks/third_party_cross_reference.md`, `external_view_blend.md`

## Executive summary

Marvin floor **13.92%** per year (infrastructure; stance **watch**) from primary filings and `valuation.json`. **Groundbreaker RE** approved 2026-06-02 (context: basin water scarcity; not a direct WBI thesis in post).

**Synthesis (best estimate):** Marvin **13.92%** base · stance **watch**; Groundbreaker supports thematic water-risk monitoring only.

## Sources in scope

| Source ID | Title | Path | Status | Cross-check status |
|-----------|-------|------|--------|-------------------|
| `groundbreaker` | Water rights — hidden asset | `WBI/research/cross_check_groundbreaker_2026-06-02.md` | approved | context |
| — | Primary filings | — | — | base IRR |

## Agreements (facts)

| Topic | Marvin (filings) | External | Source |
|-------|------------------|----------|--------|
| Base return anchor | **13.92%** per year | Qualitative support only | `WBI/research/deep_dive_2026-06-02.md` |
| Archetype / stance | **infrastructure** · **watch** | See indexed sources | `valuation.json` |
| Normalization | Merchant infrastructure — no regulated RAB. Maintenance capex estimated (~25% of growth capex); LLC/Class A/B economics approximate | Cross-check vs posts | Marvin |

## Divergences (normalization / stance)

| Topic | Marvin floor | External | Blend logic |
|-------|--------------|----------|-------------|
| Primary IRR | **13.92%** (Lawrence / scenarios) | No single approved IRR unless promoted | Marvin **70%** numeric; external **30%** catalyst timing |
| Third party | Filing-first | Context tier only | No numeric upgrade without human OK |

## Blended estimate (best judgment)

| Lens | Owner cash / value | Return / horizon | Stance hint |
|------|-------------------|------------------|-------------|
| Marvin floor | See assumption ledger | **13.92%** | **watch** |
| External (combined) | Narrative / catalyst | No change to base % | **watch** (conviction) |
| **Blended best estimate** | **Filing anchor** | **13.92%** | **watch** |

**Weights:** Marvin **70%** on numbers; indexed third party **30%** on catalyst timing and narrative (approved Substacks/HK context only in qualitative layer until human promotes).

**Returns statement (blended):** We expect **13.92%** per year at today's price on the Marvin base case; third-party sources may raise or lower conviction on timing but do not replace filing math without **[HUMAN REVIEW]**.

## [HUMAN REVIEW]

- [ ] Every **approved** source reviewed against filings
- [ ] Every **pending** source cited with **[PENDING APPROVAL]** only
- [ ] Blended estimate in `valuation.json` → `estimates.external[]` if material

## [PROPOSED MEMORY]

- [PROPOSED COMPANY] WBI: third-party cross-check fill 2026-06-02 — Marvin 13.92% unchanged; filings-only

## Primary sources cited

1. `WBI/research/deep_dive_2026-06-02.md`
2. `WBI/research/valuation.json`
3. `WBI/third-party-analyses/source_inventory_2026-06-02.md`
